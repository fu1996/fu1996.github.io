---
authors: junkui
title: JS中的jQuery的使用-03-事件-对象-插件
categories:
  - js
  - js基础回顾
tags:
...
![register](./JS中的jQuery的使用-03-事件-对象-插件/register.png)
...
![on1](./JS中的jQuery的使用-03-事件-对象-插件/on1.png)

![on2](./JS中的jQuery的使用-03-事件-对象-插件/on2.png)

![on3](./JS中的jQuery的使用-03-事件-对象-插件/on3.png)
...
> 1.点击发布按钮， 动态创建一个小 li，放入文本框的内容和删除按钮， 并且添加到 ul 中。 2.点击的删除按钮，可以删除当前的微博留言。
...
![off](./JS中的jQuery的使用-03-事件-对象-插件/off.png)
...
![t1](./JS中的jQuery的使用-03-事件-对象-插件/t1.png)

![t2](./JS中的jQuery的使用-03-事件-对象-插件/t2.png)
...
![event](./JS中的jQuery的使用-03-事件-对象-插件/event.png)
...
![extend](./JS中的jQuery的使用-03-事件-对象-插件/extend.png)
...
![noconfig](./JS中的jQuery的使用-03-事件-对象-插件/noconfig.png)
...
​ jQuery 功能比较有限，想要更复杂的特效效果，可以借助于 jQuery 插件完成。 这些插件也是依赖于 jQuery 来完成的，所以必须要先引入

jQuery 文件，因此也称为 jQuery 插件。
...
​ 我们学习的第一个插件是 jQuery 之家的开源插件，瀑布流。我们将重点详细讲解，从找到插件所在网页，然后点击下载代码，到插件的使用等，后面的插件使用可参考瀑布流插件的使用。

**下载位置**

![water](./JS中的jQuery的使用-03-事件-对象-插件/water.png)

![](./JS中的jQuery的使用-03-事件-对象-插件/download.png)

**代码演示**

​ 插件的使用三点： 1. 引入 css. 2.引入 JS 3.引入 html。 （有的简单插件只需引入 html 和 js，甚至有的只需引入 js）
...
总结：jQuery 插件就是引入别人写好的：html 、css、js （有时也可以只引入一部分，读懂后也可以修改部分内容）
...
​ 懒加载只需引入 html 和 js 操作 即可，此插件不涉及 css。
...
​ 全屏滚动因为有多重形式，所以不一样的风格 html 和 css 也不一样，但是 js 变化不大。所以下面只演示 js 的引入，html 和 css 引入根据自己实际

项目需要使用哪种风格引入对应的 HTML 和 CSS。
...
​ Bootstrap 是 Twitter 公司设计的基于 HTML、CSS、JavaScript 开发的简洁、直观、强悍的前端开发框架，他依靠 jQuery 实现，且支持响应式

布局，使得 Web 开发更加方便快捷。

​ **凡是在软件开发中用到了软件的复用，被复用的部分都可以称为组件，凡是在应用程序中已经预留接口的组件就是插件**。Bootstrap 组件使

用非常方便: 1.引入 bootstrap 相关 css 和 js 2.去官网复制 html

**代码演示**

1. 引入 bootstrap 相关 css 和 js
...
### 1.4.5. bootstrap 插件（JS）

​ bootstrap 中的 js 插件其实也是组件的一部分，只不过是需要 js 调用功能的组件，所以一般 bootstrap 的 js 插件一般会伴随着 js 代码（有的也可以

省略 js，用属性实现）。

​ 步骤： 1.引入 bootstrap 相关 css 和 js 2.去官网复制 html 3.复制 js 代码，启动 js 插件。
...
```javascript
<script>
  // 当我们点击了自己定义的按钮，就弹出模态框 $(".myBtn").on("click", function(){" "}
  {
    // alert(11);
    $("#btn").modal()
  }
...
### 1.4.6. bootstrap 案例-阿里百秀

> 1.通过调用组件实现导航栏 2.通过调用插件实现登录 3.通过调用插件标签页实现 tab 栏

​ 代码实现略。(详情参考源代码)

## 1.7. 综合案例: toDoList 案例分析（代码略）
...
![总结](./JS中的jQuery的使用-03-事件-对象-插件/总结.png)
